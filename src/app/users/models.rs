...
pub struct UsersSwitchAuthOptions {
    ...
    login: Box<dyn Fn(&UserData) -> String + Send + Sync>,
}
...
impl UsersSwitchAuthOptions {
    ...
    fn build_login_fn(login: String) -> Box<dyn Fn(&UserData) -> String + Send + Sync> {
    ...
    pub fn transform_login(&self) -> &(dyn Fn(&UserData) -> String + Send + Sync) {
        &self.login
    }
}
