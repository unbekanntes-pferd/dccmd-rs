...
pub async fn download_container(
    ...
    let base_path = node
        ...
        .ok_or(DcCmdError::InvalidPath(
            "Node has no parent path".to_string(),
        ))?
        .trim_end_matches('/')
        .to_string();
    ...
    for file in &files {
        ...
        let file_base_path = file
            ...
            .parent_path
            .ok_or(DcCmdError::InvalidPath(
                "File has no parent path".to_string(),
            ))?
            .trim_start_matches(&base_path)
            .to_string();
        ...
        targets.insert(
            ...
            target
                .to_str()
                .ok_or(DcCmdError::InvalidPath(format!(
                    "Invalid target: {}",
                    target.display()
                )))?
                .to_string(),
    ...
    download_files(
        dracoon,
        files,
        target.to_str().ok_or(DcCmdError::InvalidPath(format!(
            "Invalid target: {}",
            target.display()
        )))?,
        Some(targets),
