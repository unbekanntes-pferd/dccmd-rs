...
use crate::{
    api::{
        ...
        nodes::{models::{NodeList, NodeType, CreateFolderRequest}, Nodes, rooms::models::CreateRoomRequest, Rooms, Folders},
        Dracoon,
    },
    cmd::{
        init_dracoon,
...
use super::{models::DcCmdError, utils::strings::{format_success_message, format_error_message}};
...
pub async fn list_nodes(
    ...
    let node_list = if is_search_query(&node_name) {
        ...
        search_nodes(&dracoon, &node_name, Some(&parent_path), managed, all, offset, limit).await?
    } else {
...
async fn search_nodes(
    ...
    all: bool,
    offset: u32,
    limit: u32) -> Result<NodeList, DcCmdError> {
        let parent_id = if let Some(node_path) = node_path {
            let node = dracoon.get_node_from_path(node_path).await?;
        ...
        let mut node_list = dracoon.search_nodes(search_string, parent_id, Some(0), Some(params)).await?;
        ...
        if all && node_list.range.total > 500 {
            ...
            while offset < node_list.range.total {
                ...
                while offset < node_list.range.total {
                    ...
                    let next_node_list_req = dracoon.search_nodes(search_string, parent_id, Some(0), Some(params));
                    futures.push(next_node_list_req);
                    offset += limit;
                }
