...
use crate::{
    app::users::models::UserInfo,
    core::models::{DcCmdError, PrintFormat},
};

pub fn print_user_info(term: &Term, user_info: UserInfo) -> Result<(), DcCmdError> {
    ...
    term.write_line(&format!("► user id: {}", user_info.id))
    ...
    .map_err(|_| DcCmdError::IoError)?;
    term.write_line(&format!("► username: {}", user_info.username))
        .map_err(|_| DcCmdError::IoError)?;
    term.write_line(&format!(
        "► email: {}",
    ...
    .map_err(|_| DcCmdError::IoError)?;
    term.write_line(&format!("► last login at: {last_login}"))
        .map_err(|_| DcCmdError::IoError)?;
    term.write_line(&format!("► expire at: {expire_at}"))
        .map_err(|_| DcCmdError::IoError)?;
    term.write_line(&format!("► locked: {}", user_info.is_locked))
        .map_err(|_| DcCmdError::IoError)?;
